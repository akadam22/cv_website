...
// Apply for a job on behalf of a candidate
const applyForJob = async (userId, jobId, resumeId) => {
    const hasApplied = await hasAppliedForJob(userId, jobId);

    if (hasApplied) {
        console.log(`Candidate ${userId} has already applied for job ${jobId}.`); // Debug logging
        return;
    }
    ...
    try {
        ...
        console.log(`Applied for job ${jobId} on behalf of candidate ${userId}`); // Debug logging
    } catch (err) {
        console.error(`Error applying for job ${jobId} on behalf of candidate ${userId}: ${err}`); // Error logging
    }
};
